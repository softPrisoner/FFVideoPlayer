...
void NetStreamThread::run() {
	...
	//通过avformat_alloc_context分配内存对象
	pFormatCtx = avformat_alloc_context();

	AVDictionary* avdic = NULL;
	av_dict_set(&avdic, "buffer_size", "108000", 0);
	...
	//将解码后的YUV数据转换RGB32
	img_convert_ctx = sws_getContext(pCodecCtx->width, pCodecCtx->height, pCodecCtx->pix_fmt, outW, outH,
		AV_PIX_FMT_RGB32, SWS_BICUBIC, NULL, NULL, NULL);
	numBytes = avpicture_get_size(AV_PIX_FMT_RGB32, outW, outH);

	out_buffer = (uint8_t*)av_malloc(numBytes * sizeof(uint8_t));

	avpicture_fill((AVPicture*)pFrameRGB, out_buffer, AV_PIX_FMT_RGB32, outW, outH);
	...
	while (1) {
		...
		if (packet->stream_index == videoStream)
		{
			...
			if (got_picture)
			{
				sws_scale(img_convert_ctx,
					...
					outH, pFrameRGB->data, pFrameRGB->linesize);
				//把这个RGB数据，用QImage加载
		...
		av_free_packet(packet); //释放资源，否则内存一直上升
		msleep(0.02);
	}
